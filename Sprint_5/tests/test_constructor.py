...
def test_constructor_tab_becomes_active(driver, wait, tab_locator):
    driver.get(BASE_URL)
    ...
    if tab_locator == ConstructorLocators.BUNS_TAB:
        ...
        wait.until(
            lambda _: "tab_type_current" in sauces_tab.get_attribute("class")
        )

    driver.execute_script("arguments[0].click();", tab)

    wait.until(lambda _: "tab_type_current" in tab.get_attribute("class"))
